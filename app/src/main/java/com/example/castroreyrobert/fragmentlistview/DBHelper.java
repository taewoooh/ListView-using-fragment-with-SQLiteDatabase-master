...
public class DBHelper{
    ...
    public NoteModel addNote(String title, String note){
    ...
    public long updateNote(long idToUpdate, String title, String note){
        ContentValues contentValues = new ContentValues();
        contentValues.put(COL_TITLE, title);
        contentValues.put(COL_NOTE, note);
        contentValues.put(COL_DATE, Calendar.getInstance().getTimeInMillis()+ "");
    ...
    private NoteModel cursorToNote(Cursor res){
        NoteModel newNote = new NoteModel(res.getString(1), res.getString(2),res.getLong(0), res.getLong(4));
        return newNote;
    }
